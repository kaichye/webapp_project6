...
let make_selector = document.getElementById("make");
let model_selector = document.getElementById("model");


year_selector.addEventListener("change", yearChange);
make_selector.addEventListener("change", makeChange);
model_selector.addEventListener("change", modelChange);
...
let xhrYear = new XMLHttpRequest();
xhrYear.addEventListener("load", receivedYear);
urlYear = 'http://judah.cedarville.edu/~gallaghd/ymm/ymmdb.php?fmt=xml';
xhrYear.open("GET", urlYear, true);

//xhrYear.send(); // new 



function receivedYear() {
    let parsed = parser.parseFromString(this.responseText, "application/xml");
    let years = parsed.getElementsByTagName("year");
    for (let i = 0; i < years.length; i++) {
        let op_node = document.createElement("option");
        op_node.setAttribute("value", years[i].textContent);
        op_node.classList.add("dynamic-option");
        let textNode = document.createTextNode(years[i].textContent);
        op_node.appendChild(textNode);

        year_selector.appendChild(op_node);
    }
}

function yearChange() {
    modDefaults(1);
    make_selector.value = "default";
    model_selector.value = "default";
    modDefaults(0);

    year = $("#year option:selected").text();

    let xhrMake = new XMLHttpRequest();
    xhrMake.addEventListener("load", receivedMake);
    urlMake = urlYear + "&year=" + year;
    xhrMake.open("GET", urlMake, true);
...
function receivedMake() {
    $("#make .dynamic-option").remove();
    let parsed = parser.parseFromString(this.responseText, "application/xml");
    let makes = parsed.getElementsByTagName("make");
    for (let i = 0; i < makes.length; i++) {
        let op_node = document.createElement("option");
        op_node.setAttribute("value", makes[i].lastElementChild.textContent);
        op_node.setAttribute("id", makes[i].firstElementChild.textContent);
        op_node.classList.add("dynamic-option");
        let textNode = document.createTextNode(makes[i].lastElementChild.textContent);
        op_node.appendChild(textNode);

        make_selector.appendChild(op_node);
    }
}
...
function makeChange() {
    ...
    make = $("#make option:selected").attr("id");

    let xhrModel = new XMLHttpRequest();
    xhrModel.addEventListener("load", receivedModel);
    urlModel = urlMake + "&make=" + make;
...
function receivedModel() {
    $("#model .dynamic-option").remove();
    let parsed = parser.parseFromString(this.responseText, "application/xml");
    let models = parsed.getElementsByTagName("model");
    for (let i = 0; i < models.length; i++) {
        let op_node = document.createElement("option");
        op_node.setAttribute("value", models[i].lastElementChild.textContent);
        op_node.setAttribute("id", models[i].firstElementChild.textContent);
        op_node.classList.add("dynamic-option");
        let textNode = document.createTextNode(models[i].lastElementChild.textContent);
        op_node.appendChild(textNode);

        model_selector.appendChild(op_node);
    }
}


function modelChange() {
    model_id = $("#model option:selected").attr("id");
    model_name = $("#model option:selected").attr("value");

    let KBB = document.getElementById("KBB");
    if ( $("#kbb-display").length > 0 ) {
        $("#kbb-display").remove();
    }

    let p_node = document.createElement("p");
    p_node.setAttribute("id", "kbb-display");
    let textNode = document.createTextNode(model_id + ": " + model_name);
    p_node.appendChild(textNode);

    KBB.appendChild(p_node);
}
